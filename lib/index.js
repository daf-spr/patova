...
function setupRateLimitEventExt(server, options) {
  ...
  const extractKeyAndTakeToken = function(limitd, request, reply, type) {
    extractKey(request, reply, (err, key) =>{
      ...
      if (err) {
        onFinished(err, Date.now() - request.plugins.patova.start, { rate_limited: false, error: true });
        return reply(err);
      }

      if (!limitd) {
        onFinished(new Error('Limitd is not provided'), Date.now() - request.plugins.patova.start, { rate_limited: false, error: true });
      ...
      limitd.take(type, key, (err, currentLimitResponse) => {
        ...
        if (newMinimumLimitResponse.conformant) {
          onFinished(null, Date.now() - request.plugins.patova.start, { rate_limited: false });
        ...
        onFinished(null, Date.now() - request.plugins.patova.start, { rate_limited: true });
        reply(error);
      });
    });
  };
